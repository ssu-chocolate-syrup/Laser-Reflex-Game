...
class LaserGameServer:
    ...
    def win_effect(self, player):
        def add_win_effect_data(player, row, cols, win_effect_send_data):
            for col in cols:
                _device_id, _button_id = self.pico_outputface.input_interface(row, col)
                win_effect_item = ReturnClass(_color_type=f'p{player}', _device_id=_device_id, _button_id=_button_id)
                win_effect_send_data.append(self.return_class_utils.get_convert_dict(win_effect_item))
        win_effect_send_data = []
        add_win_effect_data(1, 1, [2, 3, 4, 7, 8, 9], win_effect_send_data)
        add_win_effect_data(2, 2, range(12), win_effect_send_data)
        add_win_effect_data(3, 3, range(12), win_effect_send_data)
        add_win_effect_data(4, 4, range(1, 11), win_effect_send_data)
        add_win_effect_data(5, 5, range(3, 9), win_effect_send_data)
        add_win_effect_data(6, 3, range(5, 7), win_effect_send_data)
